...
import { ether, ETH_ADDR, tokens, GREEN, RED, formatBalance } from '../helpers'

const account = (state) => get(state, 'web3.account')
export const accountSelector = createSelector(account, (ac) => ac)

const web3 = (state) => get(state, 'web3.connection')
export const web3Selector = createSelector(web3, (we) => we)

const token = (state) => get(state, 'token.contract')
export const tokenSelector = createSelector(token, (tk) => tk)
...
const decorateOrder = (order) => {
  ...
  tokenPrice = Math.round(tokenPrice * precission) / precission
...
const tokenPriceClass = (tokenPrice, orderId, previousOrder) => {
  ...
  // show red if price is lower than previous order
...
const decorateOrderBookOrder = (order) => {
  const orderType = order.tokenGive === ETH_ADDR ? 'buy' : 'sell'
  return {
    ...order,
    orderType,
...
const decorateMyFilledOrder = (order, account) => {
  ...
  let orderType
  if (myOrder) {
    orderType = order.tokenGive === ETH_ADDR ? 'buy' : 'sell'
  } else {
    orderType = order.tokenGive === ETH_ADDR ? 'sell' : 'buy'
  }
...
const decorateMyOpenOrder = (order, account) => {
  let orderType = order.tokenGive === ETH_ADDR ? 'buy' : 'sell'
